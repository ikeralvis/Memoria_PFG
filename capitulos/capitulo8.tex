...
Las decisiones tecnológicas de TankGo quedan recogidas en detalle en el
Capítulo~3 (Marco Tecnológico), donde se justifica la elección de cada
...
- Comprobación al arranque: si \texttt{AUTO\_ENSURE\_FRESH\_ON\_STARTUP} está activa, se
  valida la antigüedad local y se lanza una sincronización condicional.
- Job programado: un \textbf{Cloud Scheduler} ejecuta diariamente un Cloud Run Job que
  sincroniza desde la API del Ministerio y exporta un snapshot Parquet a Google Cloud
  Storage para el pipeline de ML.
- Sincronización bajo demanda: si una lectura detecta datos obsoletos y
...
responsabilidad exclusiva del Cloud Run Job periódico, separando los roles de
escritura y lectura.
